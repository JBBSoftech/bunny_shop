...
# OR for production:
MAIN_DB_URI=mongodb://username:password@your-server-ip:port/appifyours
```

#### 2. Mobile App Setup
For the mobile app to connect to your backend after building the APK:

**Option A: Update the Flutter code before building APK**
In the generated Flutter code, update the `ApiConfig.baseUrl`:
```dart
class ApiConfig {
  static const String baseUrl = 'http://your-server-ip:3000'; // Your production server URL
  static const String adminId = 'your_admin_id';
}
```

**Option B: Deploy backend to accessible server**
Make sure your backend is:
- Running on a server with a public IP or domain
- Accessible from the internet (not localhost)
- Firewall allows connections on the configured PORT

**Important Notes:**
- `localhost` or `127.0.0.1` will NOT work in production APK
- Use your server's public IP address or domain name
- Ensure PORT is open and accessible
- Test the API endpoint in a browser: `http://your-server-ip:3000/health`
